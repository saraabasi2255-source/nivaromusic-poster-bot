...
async function handleMessage(msg, env) {
  ...
  // 🛑 «انصراف» تایپی — از هر مرحله‌ای که باشیم (منتظر عنوان، منتظر خواننده،
  // منتظر لینک، منتظر تایید، منتظر ارسال به کانال) همه‌چی لغو و ریست می‌شه.
  // این چک عمداً قبل از همه‌ی منطق‌های دیگه‌ست تا کلمه‌ی «انصراف» به‌عنوان
  // عنوان/خواننده/لینک ذخیره نشه.
  if (msg.text && isCancelText(msg.text)) {
    ...
    await sendMessage(
      ...
      chatId,
      pending
        ? "❌ انصراف داده شد. همه‌چی ریست شد و هیچ‌جا پست نشد.\n\nهر وقت خواستی یه فایل صوتی 🎵 یا ویس 🎙 جدید بفرست تا از اول شروع کنیم."
        : "✅ چیزی در جریان نبود؛ بات از اول آماده‌ست.\n\nیه فایل صوتی 🎵 یا ویس 🎙 بفرست تا شروع کنیم."
    );
    return;
  }
...
async function handleIncomingVoice(msg, env) {
  ...
  if (pending && pending.status === "awaiting_publish") {
    await sendMessage(
      env,
      chatId,
      "⚠️ یه پستِ قبلی هنوز منتظر تصمیم توئه (ارسال به کانال یا انصراف). اول اون رو جواب بده — یا بنویس «انصراف» تا لغو بشه — بعد فایل جدید بفرست."
    );
    return;
  }
...
async function handleCallbackQuery(cq, env) {
  ...
  if (action === "pedit_t" || action === "pedit_p") {
    ...
    await sendMessage(
      env,
      chatId,
      field === "title" ? "اسم جدید آهنگ رو بفرست:\n(برای لغو بنویس: انصراف)" : "اسم جدید خواننده رو بفرست:\n(برای لغو بنویس: انصراف)"
    );
    return;
  }
...
async function sendVoiceFinalPreview(env, chatId, userId) {
  ...
  const body = {
    ...
    reply_markup: {
      inline_keyboard: [
        [{ text: "دانلود آهنگ", url: pending.link }],
        [
...
// کلماتی که به‌عنوان «انصراف» شناخته می‌شن (تطابقِ کامل، نه «شاملِ» کلمه؛
// تا اگه اسم آهنگی چیزی شبیه این داشت اشتباهی لغو نشه)
const CANCEL_WORDS = new Set(["انصراف", "/انصراف", "/cancel"]);

// متنِ پیام رو نرمال می‌کنه و چک می‌کنه فقط «انصراف» باشه. این حالت‌ها هم
// قبول می‌شن: «انصراف!»، «❌ انصراف»، « انصراف »، «انصـراف»، نیم‌فاصله/ی و ک عربی،
// و «/cancel@BotUsername»
function isCancelText(text) {
  if (!text) return false;
  const t = String(text)
    .replace(/[\u200B-\u200F\u202A-\u202E\uFEFF]/g, "") // نیم‌فاصله و کاراکترهای نامرئی
    .replace(/[\u064B-\u065F\u0640]/g, "") // اعراب و کشیده
    .replace(/ي/g, "ی")
    .replace(/ك/g, "ک")
    .replace(/^[^\p{L}/]+|[^\p{L}]+$/gu, "") // ایموجی/علامت‌های اول و آخر
    .replace(/@\w+$/, "") // /cancel@BotUsername
    .trim()
    .toLowerCase();
  return CANCEL_WORDS.has(t);
}
